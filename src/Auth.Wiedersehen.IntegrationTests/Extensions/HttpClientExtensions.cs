...
public static class HttpClientExtensions
{
    ...
    {
        public async Task<HttpResponseMessage> CreateUserAsync(CreateUserRequest request, HttpClientMode mode = HttpClientMode.SkipVerification)
        {
            return VerifyResponse(await client.PostAsJsonAsync("/api/v1/user", request), mode);
        }

        public async Task<HttpResponseMessage> IsEmailAvailableAsync(string email, HttpClientMode mode = HttpClientMode.SkipVerification)
        {
